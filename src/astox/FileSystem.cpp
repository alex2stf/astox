...
namespace astox {
	...
	std::string Path::resolve(std::string parentDirectory) {
		...
		int parentCount = parentTokens.size();
		int count = 0;
...
FileInfo::FileInfo(const boost::filesystem::path& boostPath) {
    ...
    #else
        std::string wsPath = _path->toString();
        processWinStat(wsPath);
    #endif
...
void FileInfo::processWinStat(
    ...
    void FileInfo::describe() {
        ...
        if (_exist) {
            ...
            astox_debug(__LINE__, __FILENAME__, "FileInfo::getCreationDate ====== %s", getCreationDate()->toString().c_str());
            astox_debug(__LINE__, __FILENAME__, "FileInfo::getLastAccessDate ==== %s", getLastAccessDate()->toString().c_str());
            astox_debug(__LINE__, __FILENAME__, "FileInfo::getLastModifiedDate == %s", getLastModifiedDate()->toString().c_str());
            astox_debug(__LINE__, __FILENAME__, "FileInfo::getStatusChangedDate = %s", getStatusChangedDate()->toString().c_str());
            astox_debug(__LINE__, __FILENAME__, "FileInfo::getFileStatusv ======= %s", getFileStatusv()->toString().c_str());
    ...
    Object * FileInfo::toObject() {
        ...
        data->addKey("path", new String(getPath()->toString()));
        data->addKey("filename", new String(getPath()->getFileName()));
        data->addKey("extension", new String(getPath()->getExtension()));
        ...
        data->addKey("fileNameNoExt", new String(getPath()->getFileNameNoExt()));
