...
#define CCT (+8)
    ...
    Date::Date(long in):Value(Syntax::JS) {


		std::time_t t = in;
		std::tm tmstrt;

		#ifdef SUPPORT_LOCALTIME_S
			localtime_s(&tmstrt, &t);
		#else
			#ifdef ASTOX_OS_GENERIC_UNIX
...
//			strStandardName = timeinfo->tm_zone;
			...
			log_stack("get _weekDay[%i, %i, %i] = %i", _day, _month, _fullYear, _weekDay);
    };
